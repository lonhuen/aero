...
class NTT:
    ...
    def orderReverse(self, poly, N_bit):
        """docstring for order"""
        for i, coeff in enumerate(poly.copy()):
    ...
    def ntt(self, poly, M, N, w):
        ...
        # if not self.isNthRootOfUnity(M, 2*N, w):
        #    print("w is not 2n-th primitive root")
        #    return None
        # preprocessing
        poly = poly.copy()
        t = 1
    ...
    # The complexity is O(N log N)
    def intt(self, poly, M, N, inv_w, inv_N):
        poly = poly.copy()
