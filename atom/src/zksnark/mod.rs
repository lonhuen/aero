...
mod offline;
mod online;
use self::offline::CircuitOffline;
...
pub struct ProverOffline {
    pub proving_key: ProvingKey<Bls12<Parameters>>,
    pub circuit: CircuitOffline,
}
impl ProverOffline {
    pub fn setup(enc_path: &str) -> Self {
        let c = CircuitOffline::new(enc_path);
        //TODO use OsRng here
        let rng = &mut test_rng();
    ...
    pub fn create_proof_in_bytes(
        ...
        for i in 0..c0.len() {
            ...
            let rr = Fr::rand(rng);
            let rs = Fr::rand(rng);
            let proof = lonhh_create_proof::<Bls12<Parameters>, CircuitOffline>(
                cs.clone(),
                &matrices,
                &self.proving_key,
                rr,
